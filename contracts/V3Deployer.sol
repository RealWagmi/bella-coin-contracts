...
contract V3Deployer is RrpRequesterV0, Ownable {
    ...
    function createGame(
        ...
        (bool start, ) = address(_diceGame).call{ value: msg.value / 2 }(
            abi.encodeWithSignature("startGame(uint256)", _initialTokenRate)
        );
        require(start);
    ...
    function transferLiquidity() external {
        (bool success, bytes memory response) = address(activeGame).call(
            abi.encodeWithSignature("sendLiquidity()")
        );
        ...
        (gameInfo.gameLiquidity, gameInfo.PTStotalSupply) = abi.decode(
            response,
            (uint256, uint256)
        );
    }
    ...
    */
    ...
    function deployTokens() external {
        ...
        for (uint i; i < length; ) {
            ...
            if (address(token.memeToken) == address(0)) {
                ...
                (uint160 sqrtPriceX96, address token_) = calculateTokenDeployParams(
                    msg.sender,
                    activeGame,
                    key
                );
        ...
        for (uint i; i < keysLength; ) {
            ...
            if (token.uniPosTokenId == 0) {
                ...
                unchecked {
                    ...
                    partOfmemeToken =
                        (((gameInfo.PTStotalSupply * token.tokenBPS) / BP) * liquidityBPS) /
                        BP;
     ...
     */
    function calculateTokenDeployParams(
        address deployer,
        address diceGame,
        bytes32 key
    ) public view returns (uint160 _sqrtPriceX96, address _token) {
        ...
        _sqrtPriceX96 = zeroForToken
            ? uint160(
                Babylonian.sqrt(
                    FullMath.mulDiv(1 << 192, gameInfo.gameLiquidity, gameInfo.PTStotalSupply)
                )
            )
            : uint160(
                Babylonian.sqrt(
                    FullMath.mulDiv(1 << 192, gameInfo.PTStotalSupply, gameInfo.gameLiquidity)
                )
            );
    }
