...
contract BellaDiceGame is RrpRequesterV0, Ownable {
    ...
    mapping(bytes32 => GameRound) private gameRounds; /* gameId --> GameRound */
    ...
    function getGameRoundInfo(bytes32 _gameId) public view returns (GameRound memory gameInfo) {
    ...
    function calculateBellaDeployParams(
        ...
        uint256 nativeTokenBalance = wrappedNativeToken.getBalance();

        _sqrtPriceX96 = zeroForBella
            ? uint160(Babylonian.sqrt(FullMath.mulDiv(1 << 192, nativeTokenBalance, total)))
            : uint160(Babylonian.sqrt(FullMath.mulDiv(1 << 192, total, nativeTokenBalance)));
    }
